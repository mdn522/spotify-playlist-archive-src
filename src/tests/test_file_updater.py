...
class TestUpdateFilesImpl(IsolatedAsyncioTestCase):
    ...
    async def test_good_alias(self) -> None:
        ...
        registry_dir = self.playlists_dir / "registry"
        registry_dir.mkdir(parents=True)
        with open(registry_dir / "foo", "w", encoding="utf-8") as f:
            f.write("alias")
        await self._update_files_impl()
    ...
    async def test_index_and_metadata_json(self, mock_logger: Mock) -> None:
        ...
        with open(self.playlists_dir / "index.md", "w", encoding="utf-8") as f:
            f.write(
                textwrap.dedent(
                    """\
                    ## Playlists \\(1\\)
    ...
    async def test_success(self) -> None:
        ...
        with open(self.playlists_dir / "registry" / "abc", "w", encoding="utf-8") as f:
            pass
